...
- **当前阶段定位**：
  - Mock 业务能力、正式数据模型和正式 Web HMI 接入已形成提交基线；2026-07-21 当前工作树完整后端回归 313/313 通过，单页浏览器验收仍待重跑；
...
- 2026-07-21 已修订新增 `Ab` 液类后仍假定“只有一个启用液类”的测试选择器，统一明确选择 `FactoryGeneral-v1`；同时按已确认业务规则将 Published Workflow 测试改为允许原地修改、删除仍禁止。实际执行 `dotnet test backend/Stainer.Tests/Stainer.Tests.csproj --no-restore`：**总计 313，313 通过、0 失败、0 跳过**。本轮测试修订尚未提交。
...
| Mock 端到端后端验收 | 当前回归通过 | HE/IHC、DAB、跨瓶、暂停恢复等能力已实现；2026-07-21 当前工作树完整测试 313/313 通过 |
| 操作员 UI 正式数据接入 | 已完成 | `/control-console` 单页中的样本、试剂、DAB 与运行视图 |
| 工程/管理/历史/报警 UI 接入 | 已完成 | 正式追溯与敏感信息分层 |
| 浏览器验收与 Mock 阶段收口 | 历史基线已完成；当前待复核 | 既有 Playwright Core、重连、多窗口、权限验收曾通过；后端回归已恢复全绿，当前单页 UI 仍需重新执行浏览器验收 |
...
- Mock 后端验收：2026-07-21 当前工作树完整回归 313/313 通过。
- 操作员 UI 正式数据接入：已完成。
...
- 主项目测试：历史基线曾为 **218/218 tests passed**；2026-07-20 一度为 252/313 通过。2026-07-21 修订测试数据选择与 Published Workflow 契约后，当前工作树完整回归为 **313/313 通过、0 失败、0 跳过**。
...
历史阶段已完成以下验收；当前后端完整回归已恢复为 313/313 通过，但 UI 已收敛到 `/control-console`，当前工作树仍须重新执行浏览器回归：
...
- 配置模板、时间线编辑和规划规则持久化已进入提交基线；Mock Workflow 执行器、运行状态机、运行账本和资源租约也已实现。当前完整后端回归 313/313 通过，迁移与浏览器验收仍需复核；Real 设备执行编排仍是独立且未完成的能力。
...
Mock Workflow 执行/调度基线（执行器、运行状态机、运行账本、资源租约、暂停/恢复/故障处理；当前完整回归 313/313 通过）
...
> 当前工作树完整后端回归已恢复为 313/313 通过，但单页浏览器验收、迁移复核和真实设备现场验收仍未完成，不能据此视为发布可用。
