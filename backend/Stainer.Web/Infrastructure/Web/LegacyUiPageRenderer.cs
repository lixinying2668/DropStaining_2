...
""";
    ...
    private static string SamplesContent()
    {
        ...
        <section class="control-strip modern-card v18-sample-actions">
          ...
          <div class="scan-hint"><b>冻结规则</b><span>同一通道 1-4 张为完整批次；启动后不得追加。</span></div>
        </section>
    ...
    {
        return """
        <section class="control-strip modern-card"><div class="control-main reagent-scan-actions"><button class="btn btn-primary btn-xl" onclick="scanReagents()">扫描全部试剂架</button><button class="btn btn-soft btn-xl" onclick="mockColumnScan(1)">扫描 ch1</button><button class="btn btn-soft btn-xl" onclick="mockColumnScan(2)">扫描 ch2</button><button class="btn btn-soft btn-xl" onclick="mockColumnScan(3)">扫描 ch3</button><button class="btn btn-soft btn-xl" onclick="mockColumnScan(4)">扫描 ch4</button><button class="btn btn-soft btn-xl" onclick="mockColumnScan(5)">扫描 ch5</button></div><div class="scan-hint"><b>启动强拦截</b><span>缺失、EMPTY、INVALID、未知、过期或余量不足均禁止启动。</span></div></section>
        <section class="split-grid wide-left reagent-v18-layout"><article class="modern-card"><div class="section-title"><div><h2>5×8 试剂架 / R1-R40</h2><p>每次列扫码返回 EMPTY / VALID / INVALID；点击瓶位查看解析结果。</p></div><div class="badge-soft" id="reagentBadge">0 个 VALID</div></div><div class="reagent-deck v18-reagent-deck" id="reagentDeck"></div></article><aside class="stack-panel"><article class="modern-card compact-card"><h3>列扫码状态</h3><div class="column-status-grid" id="columnStatus"></div></article><article class="modern-card compact-card dab-mini"><h3>DAB 临时配液区</h3><div class="dab-mini-grid"><span>M1</span><span>M2</span><span>M3</span><span>M4</span><span>M5</span><span>M6</span><span>M7</span><span>M8</span></div><button class="btn btn-soft full" onclick="toast('Mock: DAB 配液区清洗确认已记录')">确认清洗完成</button></article></aside></section>
        <div id="reagentDetail" class="modal-mask hidden"><div class="modal-card"><header><h2>试剂瓶解析详情</h2><button class="icon-btn" onclick="reagentDetail.classList.add('hidden')">×</button></header><div id="reagentDetailBody" class="detail-grid"></div><footer><button class="btn btn-soft" onclick="reagentDetail.classList.add('hidden')">关闭</button><button class="btn btn-primary" onclick="toast('Mock: 已请求重新扫码该位置')">重新扫码</button></footer></div></div>
        """;
    }
